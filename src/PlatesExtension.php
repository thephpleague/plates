    ...
    public function register(Engine $plates)
    {
        ...
        $c->add(
            'renderTemplate', function ($c) {
                ...
                $rt = array_reduce(
                    $c->get('renderTemplate.factories'), function ($rt, $create) {
                        return $create($rt);
                    }, $rt
                );
        ...
        $c->add(
            'renderTemplate.bind', function () {
                return Util\id();
            }
        );
        $c->add(
            'renderTemplate.factories', function () {
                return [];
            }
        );
        ...
        $plates->addMethods(
            ...
            'pushComposers' => function (Engine $e, $def_composer) {
                $e->getContainer()->wrapComposed(
                    'compose', function ($composed, $c) use ($def_composer) {
